...
public class TransactionProcessorUnitTest {
    ...
    public void test_nullInput() {
        Assertions.assertDoesNotThrow(() -> processor.processTransactions(null));
    ...
    public void test_emptyListInput() {
        ...
        Assertions.assertDoesNotThrow(() -> processor.processTransactions(transactions));
        verify(transactionRepository, never()).updateTransaction(any());
    }
    ...
    public void test_lotsOfLargeTransactions() {
        ...
        verify(transactionRepository, times(10)).updateTransaction(any());
    ...
    public void test_mixedTransactions() {
        ...
        verify(transactionRepository, times(9)).updateTransaction(any());
        for (Transaction transaction : transactions) {
            verify(transactionRepository, times(1)).updateTransaction(
                argThat(t -> t.id().equals(transaction.id()) && t.status() == TransactionStatus.PROCESSED));
        }
    ...
    @Test
    public void test_processTransactionsWhenRepositoryThrowsException() {
        List<Transaction> transactions = new ArrayList<>();
        transactions.add(
