...
public class TransactionProcessorIntegrationTest {
    ...
    @Test
    public void test_submitTransactionsWithExceptionInOneTransaction() {
        ...
        processor.submitTransactions(List.of(transaction1, transaction2, transaction3));

        // Ожидаем завершения обработки транзакций
        Awaitility.await().atMost(5, TimeUnit.SECONDS).untilAsserted(() -> {
            ...
            Assertions.assertTrue(
                updatedTransactions.stream()
                    .anyMatch(t -> t.id().equals("1") && t.status() == TransactionStatus.PROCESSED),
                "Транзакция 1 должна быть обработана"
            );
            Assertions.assertTrue(
                updatedTransactions.stream()
                    .anyMatch(t -> t.id().equals("3") && t.status() == TransactionStatus.PROCESSED),
    ...
    public void test_processNoPendingTransactions() {
        ...
        processor.submitTransactions(List.of(transaction1, transaction2));

        // Ожидаем и проверяем, что транзакции не были обработаны
    ...
    public void test_lotsOfLargeAndSmallTransactions() {
        ...
        processor.submitTransactions(largeTransactions);

        // Ожидаем завершения обработки всех транзакций
